...
* Erasing MUST emit `minecraft:dust` particles **in the colour actually removed**, drifting off the
  surface and falling, at the same 5-tick cadence. The colour MUST come from the texels the op
  genuinely changed — not from sampling the canvas beforehand, which cannot tell a brush over bare
  stone from a brush over paint, and which returns a colour from elsewhere on the face when the two
  differ. Where a stroke removes several colours, the commonest MUST be used.
* An erase that removes no paint MUST emit nothing, so scrubbing bare stone looks like what it is.
  Particles are client-side decoration and obey `showPaintParticles`; the erase *sound* stays
